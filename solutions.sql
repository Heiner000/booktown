...
SELECT * FROM books WHERE title LIKE '%Pyt%';
...
JOIN books ON subjects.id = books.subject_id WHERE subject = 'Computers';
...
SELECT books.title, books.id, COUNT(editions.edition) FROM books
JOIN editions ON books.id = editions.book_id GROUP BY books.title, books.id;